import { UnprovenTransaction, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
...
import { assertIsContractAddress, assertDefined, fromHex, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { UnprovenTransient, UnprovenInput, UnprovenOffer, UnprovenOutput, ContractDeploy, ContractCallsPrototype, ContractCallPrototype, communicationCommitmentRandomness, ContractState, QueryContext, StateValue, ReplaceAuthority, VerifierKeyRemove, ContractOperationVersion, VerifierKeyInsert, ContractOperationVersionedVerifierKey, MaintenanceUpdate, signData, ContractMaintenanceAuthority, partitionTranscripts, PreTranscript, LedgerParameters } from '@midnight-ntwrk/ledger';
...
const getStates = async (publicDataProvider, privateStateProvider, contractAddress, privateStateKey) => {
    ...
    assertDefined(privateState, `No private state found at private state key '${privateStateKey}'`);
...
const createUnprovenOutput = ({ coinInfo, recipient }) => recipient.is_left
    ? UnprovenOutput.new(coinInfo, recipient.left)
    : UnprovenOutput.newContractOwned(coinInfo, recipient.right);
...
const zswapStateToOffer = (zswapLocalState, params) => {
    ...
    zswapLocalState.inputs.forEach((qualifiedCoinInfo) => {
        ...
        if (unprovenOutput) {
            unprovenTransients.set(serializedCoinInfo, UnprovenTransient.newFromContractOwnedOutput(qualifiedCoinInfo, unprovenOutput));
            ...
            unprovenInputs.set(serializedCoinInfo, UnprovenInput.newContractOwned(qualifiedCoinInfo, params.contractAddress, params.zswapChainState));
        }
    });
...
const createUnprovenLedgerDeployTx = (verifierKeys, sk, contractState, zswapLocalState) => {
    ...
    return [
        ...
        new UnprovenTransaction(zswapStateToOffer(zswapLocalState), undefined, new ContractCallsPrototype().addDeploy(contractDeploy))
    ];
};
const createUnprovenLedgerCallTx = (circuitId, contractAddress, initialContractState, zswapChainState, partitionedTranscript, privateTranscriptOutputs, input, output, nextZswapLocalState) => {
    ...
    return new UnprovenTransaction(zswapStateToOffer(nextZswapLocalState, {
        contractAddress,
        zswapChainState
...
const removeVerifierKey = (operation) => new VerifierKeyRemove(operation, new ContractOperationVersion('v1'));
const insertVerifierKey = (operation, newVk) => new VerifierKeyInsert(operation, new ContractOperationVersionedVerifierKey('v1', newVk));
...
async (newAuthority) => {
    ...
    assertDefined(currentAuthority, `Signing key for private state key '${privateStateKey}' not found`);
...
const submitInsertVerifierKeyTx = async (providers, privateStateKey, contractAddress, circuitId, newVk) => {
    ...
    assertDefined(signingKey, `Signing key for private state key '${privateStateKey}' not found`);
...
const createUnprovenDeployTxFromVerifierKeys = async (verifierKeys, coinPublicKey, options) => {
    const { currentContractState, currentPrivateState, currentZswapLocalState } = options.contract.initialState(constructorContext(options.initialPrivateState, coinPublicKey), ...('args' in options ? options.args : []));
    const decodedCurrentZswapLocalState = decodeZswapLocalState(currentZswapLocalState);
...
const deployContract = async (providers, options) => {
    const unprovenDeployTxData = await createUnprovenDeployTx(providers.zkConfigProvider, providers.walletProvider, {
        ...options,
        signingKey: options.signingKey ?? sampleSigningKey()
    });
    const finalizedTxData = await submitTx(providers, {
...
const setInitialPrivateState = async (privateStateProvider, privateStateKey, initialPrivateState) => {
    ...
    assertDefined(existingPrivateState, `No existing private state found for private state key '${privateStateKey}'`);
...
export { CallTxFailedError, ContractTypeError, DeployTxFailedError, InsertVerifierKeyTxFailedError, RemoveVerifierKeyTxFailedError, ReplaceMaintenanceAuthorityTxFailedError, TxFailedError, call, createCallTxOptions, createCircuitCallTxInterface, createCircuitMaintenanceTxInterface, createCircuitMaintenanceTxInterfaces, createContractMaintenanceTxInterface, createUnprovenCallTx, createUnprovenCallTxFromInitialStates, createUnprovenDeployTx, createUnprovenDeployTxFromVerifierKeys, deployContract, findDeployedContract, getImpureCircuitIds, getStates, submitCallTx, submitInsertVerifierKeyTx, submitRemoveVerifierKeyTx, submitReplaceAuthorityTx, submitTx, verifierKeysEqual, verifyContractState };
