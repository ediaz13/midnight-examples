...
var io__namespace = /*#__PURE__*/_interopNamespaceDefault(io);
...
const LevelNotFoundError = io__namespace.type({
    code: io__namespace.literal('LEVEL_NOT_FOUND'),
    notFound: io__namespace.literal(true),
    status: io__namespace.literal(404)
});
...
const subLevelMaybeGet = async (dbName, levelName, key) => {
    try {
        return await withSubLevel(dbName, levelName, (subLevel) => subLevel.get(key));
    }
    catch (e) {
...
const levelPrivateStateProvider = (partialConfig = {}) => {
    ...
    return {
        get(key) {
            return subLevelMaybeGet(config.midnightDbName, config.privateStateStoreName, key);
        },
        remove(key) {
            return withSubLevel(config.midnightDbName, config.privateStateStoreName, (subLevel) => subLevel.del(key));
        },
        set(key, state) {
            return withSubLevel(config.midnightDbName, config.privateStateStoreName, (subLevel) => subLevel.put(key, state));
        },
        clear() {
            return withSubLevel(config.midnightDbName, config.privateStateStoreName, (subLevel) => subLevel.clear());
        },
        getSigningKey(key) {
            return subLevelMaybeGet(config.midnightDbName, config.signingKeyStoreName, key);
        },
        removeSigningKey(key) {
            return withSubLevel(config.midnightDbName, config.signingKeyStoreName, (subLevel) => subLevel.del(key));
        },
        setSigningKey(key, signingKey) {
            return withSubLevel(config.midnightDbName, config.signingKeyStoreName, (subLevel) => subLevel.put(key, signingKey));
        },
        clearSigningKeys() {
