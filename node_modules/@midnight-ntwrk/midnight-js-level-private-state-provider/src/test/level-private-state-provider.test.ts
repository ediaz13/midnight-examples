...
import type { PrivateStateKey } from '@midnight-ntwrk/midnight-js-types';
...
describe('Level Private State Provider', (): void => {
  ...
  type ObjectValue = {
    ...
    uint8ArrayArrayValue: Uint8Array[];
    bufferValue: Buffer;
  ...
  test("'get' returns null if key does not exist", async () => {
    const db = levelPrivateStateProvider<TestStates>();
  ...
  async function testSetGet<K extends PrivateStateKey<TestStates>>(key: K): Promise<void> {
    const stateRepo = levelPrivateStateProvider<TestStates>();
  ...
  async function testSetGetSigningKey<K extends PrivateStateKey<TestStates>>(
    key: K
  ): Promise<void> {
    const stateRepo = levelPrivateStateProvider<TestStates>();
  ...
  test("'get' throws error on non-'LEVEL_NOT_FOUND_ERROR' codes", () => {
    expect.assertions(1);
    return levelPrivateStateProvider<TestStates>()
      .get(null as unknown as keyof TestStates)
  ...
  test("'getSigningKey' throws error on non-'LEVEL_NOT_FOUND_ERROR' codes", () => {
    expect.assertions(1);
    return levelPrivateStateProvider<TestStates>()
      .getSigningKey(null as unknown as keyof TestStates)
      .catch((e) => expect(e.code).toMatch('LEVEL_INVALID_KEY'));
  });
});
